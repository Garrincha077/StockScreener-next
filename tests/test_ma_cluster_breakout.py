...
def make_bars(weeks=45, breakout=False, volume_mult=1.0, spread_bias=0.0):
    ...
    for w in range(weeks):
        # Long flattening structure with only a mild drift.
...
def test_low_volume_breakout_is_not_entry():
...
def test_ready_state_can_exist_before_breakout():
    result = score_bars(make_bars(breakout=False))
    assert result["maClusterEntrySignal"] is False
    assert isinstance(result["maClusterReady"], bool)
