...
test('Phase 4 review scope, queue, ticker sync and Rapid Review work across viewports',async({page},testInfo)=>{
  ...
  const healthPanel=page.locator('.sdh-panel')
  ...
  await expect(scope).toContainText('0 / 3 reviewed')
  await expect(scope).toContainText('screen membership paused')
  await expect(summary).toContainText('3 of 3')
  await expect(cards).toHaveCount(3)

  await scope.getByRole('button',{name:'Start review'}).click()
  const why=page.locator('.p4-why')
  const continuation=page.locator('.p4-queue-continuation')
  ...
  await why.getByRole('button',{name:'Add T001 to watchlist'}).click()
  await expect(why.getByRole('button',{name:'Remove T001 from watchlist'})).toContainText('Watched')
  await expect(page.locator('.dv-minicard.selected .dv-heart')).toHaveClass(/on/)
  expect(await page.evaluate(()=>localStorage.getItem('stockscout-watchlist'))).toBe('["T001"]')
  ...
  await expect(continuation).toContainText('Review queue · Today / changed')
  await expect(continuation).toContainText('T001 · 1 / 3')
  const detailWatch=page.locator('.dv-detail .dv-star.big')
  await expect(detailWatch).toHaveClass(/on/)
  await continuation.getByRole('button',{name:'Continue review'}).click()
  await expect(why).toContainText('WHY T001?')
  await expect(why).toContainText('Review 1 / 3')
  await expect(why.getByRole('button',{name:'Remove T001 from watchlist'})).toBeVisible()
  ...
  await expect(scope).toContainText('1 / 1 reviewed')
  await expect(scope.getByRole('button',{name:'Review again'})).toBeVisible()
