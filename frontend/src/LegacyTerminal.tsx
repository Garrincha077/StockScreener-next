  ...
  return <div className="lg-app">
    ...
    <main className="lg-work">
      ...
      <aside className="lg-detail">{detailError?<div className="lg-empty">Detail shard failed: {detailError} <button onClick={()=>indexRow&&setDetailRetry({ticker:indexRow.ticker,nonce:Date.now()})}>Retry</button></div>:!e?<div className="lg-empty">Loading LEGACY detail shard…</div>:<>
        ...
        <div className="lg-kpis"><K l="Run BUY" v={e.buy?.emittedByOriginalRun?'EMITTED':e.buy?.isBuy?'RAW ONLY':'NO'} good={e.buy?.emittedByOriginalRun}/><K l="Run SELL" v={e.sell?.emittedByOriginalRun?'EMITTED':e.sell?.isSell?'RAW ONLY':'NO'} good={e.sell?.emittedByOriginalRun}/><K l="TT" v={`${fmt(e.minervini?.passed,0)}/8`} good={e.minervini?.passes}/><K l="VCP" v={fmt(e.vcp?.quality,0)}/><K l="Assumed entry" v={money(row?.price)}/><K l="Pivot" v={money(e.breakout?.breakout_level)}/><K l="Stop" v={money(e.buy?.stopLoss)}/><K l="Risk" v={pct(e.buy?.riskPct)}/><K l="Target" v={money(e.buy?.rewardTarget)}/><K l="R/R" v={`${fmt(e.buy?.riskReward,1)}:1`} good={(e.buy?.riskReward||0)>=2}/><K l="Entry quality" v={e.buy?.entryQuality||'—'}/></div>
        <section><h3>ORIGINAL TRADE GEOMETRY · SOURCE ASSUMPTION</h3><p>Ryan calculates risk/reward from the scan/current price as the entry reference. The pivot/breakout level is shown separately when the frozen source detected one; it is not a second execution price.</p><div className="lg-components"><K l="Assumed entry" v={money(row?.price)}/><K l="Pivot / breakout" v={money(e.breakout?.breakout_level)}/><K l="Stop" v={money(e.buy?.stopLoss)}/><K l="Target" v={money(e.buy?.rewardTarget)}/><K l="Risk $" v={money(buyDetails.risk_amount)}/><K l="Reward $" v={money(buyDetails.reward_amount)}/><K l="R/R" v={`${fmt(e.buy?.riskReward,2)}:1`} good={(e.buy?.riskReward||0)>=2}/><K l="Entry quality" v={buyDetails.entry_score!=null?`${e.buy?.entryQuality||'—'} · ${fmt(buyDetails.entry_score,2)}/5`:e.buy?.entryQuality||'—'}/></div></section>
        ...
        <section><h3>BREAKOUT / VOLUME</h3><div className="lg-components"><K l="Type" v={e.breakout?.breakout_type||'—'}/><K l="Level" v={money(e.breakout?.breakout_level)}/><K l="Volume" v={e.breakout?.volume_confirmed?'CONFIRMED':'not confirmed'} good={e.breakout?.volume_confirmed}/><K l="A/D vol" v={ratio(e.buy?.adVolumeRatio)}/></div></section>
        <section><h3>VCP ANATOMY</h3><p>{e.vcp?.pattern||'No source VCP pattern description.'}</p>{contractions.length>0&&<div className="lg-contract">{contractions.map((c:any,i:number)=><span key={i}><b>C{i+1}</b>{pct(c.drawdownPct)}<small>{c.durationDays||0}d · vol {ratio(c.volumeRatio)}</small></span>)}</div>}</section>
        <section><h3>RYAN ORIGINAL DIAGNOSTICS · READ ONLY</h3><p>Direct fields from the frozen source output. No rescoring or StockScout input.</p><div className="lg-components"><K l="RS slope" v={fmt(buyDetails.rs_slope,3)}/><K l="Up-day vol" v={compact(buyDetails.avg_vol_up)}/><K l="Down-day vol" v={compact(buyDetails.avg_vol_down)}/><K l="R/R score" v={`${fmt(buyDetails.rr_score,2)}/15`}/><K l="VCP base" v={typeof (vcpSource.base_length_weeks??e.vcp?.baseLengthWeeks)==='number'?`${fmt(vcpSource.base_length_weeks??e.vcp?.baseLengthWeeks,1)}w`:'—'}/><K l="VCP vol ratio" v={ratio(vcpSource.volume_ratio)}/></div>{vcpFactors.length>0&&<><h3>VCP QUALITY FACTORS · SOURCE</h3><ul>{vcpFactors.map((factor:any,i:number)=><li key={i}>{sourceText(factor)}</li>)}</ul></>}</section>
        ...
        {(e.sell?.score||0)>0||e.sell?.sourceReason?<section className="lg-sell"><h3>SELL / RISK ENGINE</h3><b>{fmt(e.sell?.score,0)} · {String(e.sell?.severity||'none').toUpperCase()}</b>{failedBreakout&&<> <span className="pill bad">FAILED BREAKOUT</span></>}<div className="lg-components"><K l="Breakdown" v={`${fmt(sellDetails.breakdown_score,1)}/60`}/><K l="Volume" v={`${fmt(sellDetails.volume_score,1)}/30`}/><K l="RS weakness" v={`${fmt(sellDetails.rs_score,1)}/10`}/><K l="RS slope" v={fmt(sellDetails.rs_slope,3)}/><K l="Vol ratio" v={ratio(sellDetails.volume_ratio)}/><K l="Breakdown level" v={money(sellDetails.breakdown_level)}/></div>{e.sell?.sourceReason&&<p>{e.sell.sourceReason}</p>}<ul>{(e.sell?.reasons||[]).map((r:string,i:number)=><li key={i}>{r}</li>)}</ul></section>:null}
