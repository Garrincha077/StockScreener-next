...
test('drawings live on the main chart, preserve tool kind, project forward, edit there and persist across reload',async({page})=>{
  ...
  await expect(manager).toContainText('manages rules and status')
  await expect.poll(()=>page.evaluate(()=>document.body.classList.contains('cad-manager-open'))).toBe(true)
  if((page.viewportSize()?.width||0)>900){
    await expect.poll(()=>page.evaluate(()=>parseFloat(getComputedStyle(document.querySelector('.dv-app')!).paddingRight))).toBeGreaterThan(350)
  }

  const horizontalRow=manager.locator('.cad-manager-row').filter({hasText:'Level'}).first()
  await horizontalRow.click()
  await expect(horizontalRow).toHaveClass(/selected/)
  const selected=manager.getByRole('region',{name:'Selected drawing alert settings'})
  await expect(selected).toContainText('Level')
  await selected.getByLabel(`${ticker} alert condition`).selectOption('touch')
  await expect.poll(()=>rules.find(item=>item.drawing_id==='d2')?.condition).toBe('touch')
  await expect(selected.getByLabel(`${ticker} alert source`)).toHaveValue('wick')
  await selected.getByLabel(`${ticker} alert lifecycle`).selectOption('one_shot')
  await expect.poll(()=>rules.find(item=>item.drawing_id==='d2')?.lifecycle).toBe('one_shot')
  await selected.getByLabel('Telegram').uncheck()
  await expect.poll(()=>rules.find(item=>item.drawing_id==='d2')?.notify_telegram).toBe(false)
  await expect(selected.getByRole('button',{name:'Delete alert rule'})).toBeVisible()
  await manager.getByRole('button',{name:'Close drawings and alerts'}).click()
  await expect.poll(()=>page.evaluate(()=>document.body.classList.contains('cad-manager-open'))).toBe(false)
