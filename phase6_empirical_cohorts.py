...
def normalized_status(row: dict[str, Any]) -> str:
    return str(row.get("legacyConfirmationStatus") or "").strip().upper()


def is_strong_stockscout(row: dict[str, Any]) -> bool:
...
def is_low_stockscout(row: dict[str, Any]) -> bool:
...
def has_legacy_buy(row: dict[str, Any]) -> bool:
    """Consume the already-captured frozen original-run BUY output only."""
    return row.get("originalRunBuySignal") is True


def classify_row(row: dict[str, Any]) -> tuple[str, ...]:
    """Return zero or more roadmap cohort memberships for one row.

    The four cohorts are evaluated independently. Current definitions are
    mutually exclusive for ordinary captured data, but the tuple contract avoids
    baking that assumption into the journal format.
    """
    cohorts: list[str] = []
...
def _observation(row: dict[str, Any], cohort: str, generated_at: str) -> dict[str, Any]:
    return {
        ...
        "legacyConfirmationStatus": normalized_status(row),
...
def build_snapshot_observations(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    for row in rows:
        ...
        if not ticker:
            continue
        for cohort in classify_row(row):
            observations.append(_observation(row, cohort, generated_at))
            counts[cohort] += 1
    ...
    result = {
        ...
        "definitions": {
            ...
            "legacyConfirmationAffectsStockScout": False,
        },
        "cohortCounts": counts,
