...
const defaultVisibility:VisibilityState={originalTTPasses:false,originalVcpQuality:false,originalAdVolumeRatio:false,originalRiskPct:false,originalSellScore:false,rsFromHigh:false,volumeDryUp:false,baseWeeks:false,distance30w:false,structureScore:false,baseScore:false,triggerScore:false,neglectedScore:false,avgDollarVolume20:false,fundamentalSupport:false,fundamentalEvidenceConfidence:false,fundamentalEvidenceCoverage:false,leadershipScore:false,groupRank:false,groupRS:false,groupConfidence:false}
...
function Detail({stock,bars,loading,interval,setInterval,range,setRange,mode,setMode,watched,toggleWatch}:{stock:Stock;bars:Bar[];loading:boolean;interval:Interval;setInterval:(v:Interval)=>void;range:Range;setRange:(v:Range)=>void;mode:ChartMode;setMode:(v:ChartMode)=>void;watched:boolean;toggleWatch:()=>void}){
  ...
  return <aside className="dv-detail"><div className="dv-detailhead"><button className={`dv-star big ${watched?'on':''}`} onClick={toggleWatch}>★</button><div><h1>{stock.ticker}</h1><span>Stage {stock.stage} · {stock.stageName}</span></div><div className="dv-opp"><small>OPPORTUNITY</small><b>{opp(stock)}</b>{num(stock.opportunityDelta)!==0&&<span className={num(stock.opportunityDelta)>0?'dv-good':'dv-bad'}>{signed(stock.opportunityDelta,0)}</span>}</div><div className="dv-price"><b>${fmt(stock.price,2)}</b><span>{signed(stock.change20d)} 20D</span></div></div>
    ...
    <div className="dv-kpis"><K l="RS Rank" v={fmt(stock.rsRank,0)} d={stock.rsRankDelta}/><K l="RS Δ" v={fmt(stock.rsAcceleration,2)}/><K l="Group" v={fmt(stock.groupRank,0)}/><K l="Group RS" v={signed(stock.groupRS)}/><K l="Group conf" v={`${fmt(stock.groupConfidence,0)}%`}/><K l="TT" v={`${fmt(stock.trendTemplatePasses,0)}/8`}/><K l="S2 age" v={`${fmt(stock.stage2AgeWeeks,1)}w`}/><K l="Vol" v={`${fmt(stock.volumeRatio,2)}x`} d={stock.volumeRatioDelta}/><K l="Breakout" v={signed(stock.breakoutPct)}/><K l="10W" v={signed(stock.distance10w)}/><K l="30W" v={signed(stock.distance30w)}/><K l="Fund Ev" v={fmt(stock.fundamentalEvidenceScore,0)}/><K l="Fund conf" v={`${fmt(stock.fundamentalEvidenceConfidence,0)}%`}/></div>
