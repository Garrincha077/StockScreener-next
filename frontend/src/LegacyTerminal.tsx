...
export default function LegacyTerminal(){
  ...
  return <div className="lg-app">
    ...
    <main className="lg-work">
      ...
      </tr></thead><tbody>{filtered.map(r=><tr key={r.ticker} className={r.ticker===row?.ticker?'selected':''} onClick={()=>setSelected(r.ticker)}><td><b>{r.ticker}</b><small>{money(r.price)}</small></td><td>{r.originalEngine?.phase??r.stage??'—'}</td><td><strong>{fmt(r.originalBuyScore,0)}</strong></td><td>{r.originalRunBuySignal||r.originalMarketQualifiedBuy?<span className="pill good">EMITTED</span>:r.originalEngine?.buy?.isBuy?<span className="pill">RAW BUY</span>:'—'}</td><td>{fmt(r.originalTTPasses,0)}/8</td><td>{fmt(r.originalVcpQuality,0)}</td><td>{fmt(r.originalRR,1)}:1</td><td>{r.originalEntryQuality||'—'}</td><td>{fmt(r.originalAdVolumeRatio,2)}x</td><td>{r.originalBreakoutType||'—'}</td><td className={r.originalSell?'bad':''}>{fmt(r.originalSellScore,0)}</td><td>{r.originalRunSellSignal||r.originalMarketQualifiedSell?<span className="pill good">EMITTED</span>:r.originalSell?<span className="pill">RAW SELL</span>:'—'}</td></tr>)}</tbody></table></div></section>
