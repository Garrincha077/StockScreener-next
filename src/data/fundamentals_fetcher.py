...
def _extended_series(frame: pd.DataFrame, *names: str):
    """Return first available statement row without changing legacy calculations."""
...
def _extended_change(values, periods: int):
    if values is None:
        return None
    clean = values.dropna()
    if len(clean) <= periods:
        return None
    latest = _extended_finite(clean.iloc[-1])
    previous = _extended_finite(clean.iloc[-periods - 1])
    if latest is None or previous in (None, 0):
        return None
    return ((latest - previous) / abs(previous)) * 100


def _add_unused_upstream_statement_metrics(
    result: Dict[str, any],
    quarterly_income: pd.DataFrame,
    quarterly_balance: pd.DataFrame,
    quarterly_cashflow: pd.DataFrame,
) -> None:
    """Expose additive metrics from yfinance statements the upstream already downloads.

    This helper never mutates any legacy key used by the original scoring path.
    Missing new metrics simply remain absent.
    """
    result['fundamental_data_source'] = 'yfinance'

    revenue = _extended_series(quarterly_income, 'Total Revenue')
    ocf = _extended_series(
        quarterly_cashflow,
        'Operating Cash Flow',
        'Total Cash From Operating Activities',
    )
    capex = _extended_series(
        quarterly_cashflow,
        'Capital Expenditure',
        'Capital Expenditures',
    )
    fcf = _extended_series(quarterly_cashflow, 'Free Cash Flow')

    if ocf is not None:
        result['quarterly_operating_cash_flow'] = ocf.to_dict()
        result['operating_cash_flow_qoq_change'] = _extended_change(ocf, 1)
        result['operating_cash_flow_yoy_change'] = _extended_change(ocf, 4)
    ...
    if fcf is not None:
        ...
        result['free_cash_flow_qoq_change'] = _extended_change(fcf, 1)
        result['free_cash_flow_yoy_change'] = _extended_change(fcf, 4)
        latest_fcf = _extended_finite(fcf.dropna().iloc[-1]) if len(fcf.dropna()) else None
        latest_revenue = _extended_finite(revenue.dropna().iloc[-1]) if revenue is not None and len(revenue.dropna()) else None
        result['free_cash_flow_margin'] = (
            (latest_fcf / latest_revenue * 100)
            if latest_fcf is not None and latest_revenue not in (None, 0)
            else None
        )

    cash = _extended_series(
        quarterly_balance,
        'Cash Cash Equivalents And Short Term Investments',
        'Cash And Cash Equivalents',
        'Cash',
    )
    debt = _extended_series(
        quarterly_balance,
        'Total Debt',
        'Long Term Debt And Capital Lease Obligation',
        'Long Term Debt',
    )
    shares = _extended_series(
        quarterly_balance,
        'Ordinary Shares Number',
        'Share Issued',
    )

    latest_cash = _extended_finite(cash.dropna().iloc[-1]) if cash is not None and len(cash.dropna()) else None
    latest_debt = _extended_finite(debt.dropna().iloc[-1]) if debt is not None and len(debt.dropna()) else None
    ...
    if debt is not None:
        ...
        result['total_debt_qoq_change'] = _extended_change(debt, 1)
        result['total_debt_yoy_change'] = _extended_change(debt, 4)
    if latest_cash is not None or latest_debt is not None:
        result['net_debt_latest'] = (latest_debt or 0) - (latest_cash or 0)

    if shares is not None:
        result['quarterly_shares_outstanding'] = shares.to_dict()
        result['shares_outstanding_latest'] = _extended_finite(shares.dropna().iloc[-1]) if len(shares.dropna()) else None
        result['share_dilution_yoy_change'] = _extended_change(shares, 4)
...
def fetch_quarterly_financials(ticker: str) -> Dict[str, any]:
    ...
    try:
        ...
        # Fork-only, additive use of the cash-flow/balance-sheet frames that the
        # original implementation already fetched but mostly ignored.
        _add_unused_upstream_statement_metrics(
            result,
            quarterly_income,
            quarterly_balance,
            quarterly_cashflow,
